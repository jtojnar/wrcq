...
passport.deserializeUser(function(id, done) {
	pg.connect(_dbUri, function(err, client) {
		...
		dabin(client, 'select * from "user" where "id"=$1', [id], function(result) {
	...
	function(email, password, done) {
		process.nextTick(function () {
			pg.connect(_dbUri, function(err, client) {
				...
				dabin(client, 'select * from "user" where "email"=$1', [email], function(result) {
...
var pg = require('pg'), dabin = require('./dabin').dabin;
...
app.get('/', function(req, res) {
	pg.connect(_dbUri, function(err, client) {
		dabin(client, 'select * from update order by timestamp desc limit 10', function(result) {
			var updates = result.rows;
			dabin(client, 'select * from event order by start asc', function(result) {
				res.render('index', {updates: updates, events: result.rows, identity: req.user});
			});
		});
	});
});
...
app.get('/results', function(req, res) {
	...
	pg.connect(_dbUri, function(err, client) {
		dabin(client, 'select * from member where event_id=$1', [event.id], function(result) {
			var members = result.rows;
			var query = dabin(client, 'select * from team where event_id=$1', [event.id], function(result) {
				res.render('results', {
