...
router.post('/api/update/add', async function(req, res) {
	...
	try {
		let data = await db.query(sql`insert into update(text, user_id, timestamp) values(${req.body.content}, ${req.user.id}, ${new Date()}) returning id`);
...
router.post('/api/update/edit', async function(req, res) {
	...
	try {
		await db.query(sql`update update set text = ${req.body.content} where id = ${id}`);
...
router.get('/qualified', async function(req, res) {
	let eventIds = {};
	let eventsQuery = await db.query(sql`select * from event`);
